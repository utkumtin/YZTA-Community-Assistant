...
- **`/cemil-report feature-requests`**
  Admine özel raporlama komutudur. Çalıştırıldığında arka planda `run_clustering_pipeline()` tetiklenerek bekleyen talepler (AI/LLM desteği ile) kümelenir ve kanal/kullanıcı bazlı detaylı bir admin raporu (`Layouts.feature_request_report`) DM üzerinden sunulur. Yalnızca `slack_admins` yapılandırmasındaki kullanıcılar çalıştırabilir.

### Modallar (View Submissions)

- **`feature_request_modal`** 
  Kullanıcı `/cemilimyapar` pop-up'ını doldurup gönderdiğinde devreye girer. Üç temel durumu yönetir:
  1. **created**: Talep yeniyse başarıyla veritabanına kaydedilir.
  2. **similar_found**: Gönderilen metin vektörel/anlamsal olarak öncekilerle benzerlik gösteriyorsa kullanıcı uyarılır. Aynı veya benzer içerik yerine düzenlemesi (edit) teşvik edilir.
  3. **quota_exceeded**: Kullanıcının aylık özellik talep etme limiti dolduysa işlemini reddeder (Örn: `X/Y hakkınız doldu`).
...
- **`feature_edit_yes`**: Benzer kayıt ("similar_found") loglandığında kullanıcının karşısına çıkan "Düzenle" butonudur. Tıklandığında mevcut talebin ID'si ile beraber `feature_request_edit_modal` arayüzünü açar.
- **`feature_edit_no`**: Kullanıcının benzer talep uyarısı sonrası düzenleme yapmaktan vazgeçtiğini gösteren iptal butonudur. İşlemi iptal edip bilgilendirme mesajı gönderir.
